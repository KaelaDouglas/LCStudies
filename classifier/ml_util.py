...
##### THIS ONE HAS MY NOTES / EXPLANATION IN IT!!! #####
...
def splitFrameTVT(frame, trainlabel='train', trainfrac = 0.8, testlabel='test', testfrac = 0.2, vallabel='val'):
    ...
    train_index, testval_index = next(train_split.split(frame))  #need next basically to execute the shufflesplit function

    if valfrac > 0:
        testval_split = ShuffleSplit(#shufflesplit returns a list of indices? 
    ...
    frame[trainlabel] = frame.index.isin(train_index) #this gives like a T/F for if a given indes in frame is one of the training ones (or test or val below) then use pandas masking to return the subset of events that are part of train (or test or val) and add that as a new variable in the frame :))
...
def setupCells(tree, layer, nrows = -1, indices = [], flatten=True):
    ...
    if nrows > 0:
        ...
        array = array[indices] #select only specific indices
    num_pixels = cell_meta[layer]['len_phi'] * cell_meta[layer]['len_eta'] #just for the length if it were to be flattened
    if flatten:
...
def standardCells(array, layer, nrows = -1):
    ...
    if type(layer) == str:
        ...
        print('you should not be here') #lol
...
def standardCellsGeneral(array, nrows = -1):
    ...
    flat_array = np.array(working_array.reshape(total, 1)) #isn't there a flatten function that'll do this for you?
    #yes probably - could be a nicer way to do this, just that scaler needs a vector as input and so need to flatten, and get back to original shape at the end that's all lol
